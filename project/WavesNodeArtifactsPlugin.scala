...
object WavesNodeArtifactsPlugin extends AutoPlugin {
    ...
    downloadWavesNodeArtifacts := {
      ...
      if (isOldWavesVersion.value) {
        ...
        val grpcServerArchive = targetDir.listFiles(new FileFilter {
          override def accept(pathname: File): Boolean = pathname.name.matches("decentralchain-grpc-server.+\\.tgz")
        }).headOption.getOrElse(throw new RuntimeException("Can't find a decentralchain-grpc-server archive"))
  ...
  private def artifactNames(version: String): List[List[String]] = List(
    ...
    networkSuffixes.map(x => s"decentralchain-grpc-server$x-$version.tgz"),
    networkSuffixes.map(x => s"decentralchain-grpc-server${x}_${version}_all.deb")
  )
